...
MODEL_PATH = os.path.join(dir_of_interest, 'diabites.pkl')
...
if btn_click == True:
    if preg and glucose and bp and stk and insulin and bmi and dpf and age:
        query_point = np.array([int(preg), float(glucose), float(bp),float(stk),float(insulin),float(bmi),float(dpf),int(age)]).reshape(1,-1)
